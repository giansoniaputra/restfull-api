...
class AddressController extends Controller
{
    ...
    public function get(int $idContact, int $idAdress): AddressResource
    {
        ...
        $contact = Contact::where('user_id', $user->id)->where('id', $idContact)->first();
        if (!$contact) {
            throw new HttpResponseException(response()->json(['error' => ['message' => 'Contact Not Found']])->setStatusCode(404));
        }
        $address = Address::where('contact_id', $idContact)->where('id', $idAdress)->first();
        if (!$address) {
            throw new HttpResponseException(response()->json(['error' => ['message' => 'Address Not Found']])->setStatusCode(404));
        }
    ...
    public function update(int $idContact, int $idAddress, AddressUpdateRequest $request): AddressResource
    {
        ...
        $address = Address::where('id', $idAddress)->first();
        if (!$address) {
            throw new HttpResponseException(response()->json(['error' => ['message' => 'Address Not Found']])->setStatusCode(404));
        }
    ...
    public function delete(int $idContact, $idAddress): JsonResponse
    {
        $user = Auth::user();
        $contact = Contact::where('user_id', $user->id)->where('id', $idContact)->first();
        if (!$contact) {
